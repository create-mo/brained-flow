#!/usr/bin/env bash
# install-macos.sh — brained-flow installer for macOS
set -e
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo ""
echo "  brained-flow installer (macOS)"
echo "  ─────────────────────────────────────────"
echo ""

# ── 1. Cowork skills directory ────────────────────────────────────────────────

DEFAULT_SKILLS="$HOME/Library/Application Support/Claude/skills"
...
if [ -n "$BRAIN_DIR" ]; then
    ...
    if ! command -v fswatch &>/dev/null; then
        echo ""
        echo "  fswatch not found — required for wiki-watch.sh"
